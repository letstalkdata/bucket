...
EOF
#
mysqlsh -- dba configure-instance { --port=3306 --host=localhost --password='Bucket1!sql' --user=root } --mycnfPath='/root/mysql.conf' --interactive=false >> /root/mysqlInit.log 2>&1
systemctl restart mysqld 
mysqlsh -- dba check-instance-configuration { --port=3306 --host=localhost --password='Bucket1!sql' --user=root } 